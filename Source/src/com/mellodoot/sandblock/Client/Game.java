package com.mellodoot.sandblock.Client;
...
import com.mellodoot.sandblock.Client.Entity.Mob.*;
import com.mellodoot.sandblock.Client.Graphics.*;
import com.mellodoot.sandblock.Client.Graphics.Font;
import com.mellodoot.sandblock.Client.Input.*;
import com.mellodoot.sandblock.Client.UI.*;
import com.mellodoot.sandblock.Client.UI.Menu;
import com.mellodoot.sandblock.Client.World.*;
import com.mellodoot.sandblock.Server.*;
...
public class Game extends Canvas implements Runnable {
			...
			if (!world.loading) {
				...
				if (key.c) {
					for (Entity e: world.entities) {
						if (e instanceof ClientPlayer) continue;
						e.remove();
					}
				}

				player.update(key.w, key.s, key.a, key.d, key.space, key.ctrl, key.shift, key.m, mouse.getX() - width / 2, mouse.getY() - height / 2, mouse.getButton() == 3);
			}
	...
	public void save(World world) {
		SerialDatabase db = new SerialDatabase(world.name);
		SerialObject tilesObj = new SerialObject("Tiles");
		SerialObject worldObj = new SerialObject("World");
		
		SerialField width = SerialField.Integer("Width", world.width);
		SerialField height = SerialField.Integer("Height", world.height);
		SerialField spawnX = SerialField.Integer("Width", world.spawnX);
		SerialField spawnY = SerialField.Integer("Height", world.spawnY);
		...
		SerialField time = SerialField.Integer("Time", world.time);
	...
	public World load(String name) {
		...
		SerialDatabase db = SerialDatabase.deserializeFromFile(this.dir + dir);
		if (db == null) return null;
		SerialObject tilesObj = db.findObject("Tiles");
		SerialObject worldObj = db.findObject("World");
	...
	public void newApi(String user, String token) {
		...
		if (api.verifyUser(user, token)) {
			...
			if (api.isVerified()) {
				...
				SerialDatabase db = new SerialDatabase("Connect");
				SerialObject obj = new SerialObject("Data");
				SerialField version = SerialField.Float("Version", Game.version);
	...
	public void saveConfig() {
		SerialDatabase db = new SerialDatabase("Config");
		SerialObject obj = new SerialObject("Screen");
		obj.addField(SerialField.Integer("width", actualWidth));
		obj.addField(SerialField.Integer("height", actualHeight));
		obj.addField(SerialField.Integer("scale", scale));
		db.addObject(obj);
	...
	public void loadConfig() {
		try {
			SerialDatabase db = SerialDatabase.deserializeFromFile(this.dir + "config.bin");
			SerialObject obj = db.findObject("Screen");
